...
Plattform verstanden, auf der mehrere Rekonstruktionsverfahren eine gemeinsame
Streaming-Pipeline nutzen. Ziel ist es, Bild- und Posendaten aus einer Unity-basierten
...
\begin{itemize}
    ...
    \item \textbf{Wiederverwendbarkeit im Lab:} RTReconstruct soll als generische Infrastrukturkomponente
    betrachtet werden können, die in unterschiedlichen Va.Si.Li-Szenarien
...
Rekonstruktionsergebnisse werden fortlaufend an die Clients zurückgegeben.
...
\begin{itemize}
    \item \textbf{Verbindungsaufbau}: Das Frontend etabliert eine persistente, bidirektionale Verbindung zum Router und erhält im Handshake die Liste verfügbarer Rekonstruktionsmodelle.
    ...
    \item \textbf{Verteilung und Visualisierung}: Der Router hält für jede Szene und jedes Modell das aktuellste Rekonstruktionsergebnis vor und verteilt es an alle Clients dieser Szene. Das Frontend integriert die empfangenen 3D-Repräsentationen in die laufende VR-Szene.
\end{itemize}
...
hergeleitet. Im Zentrum steht ein VR-Frontend in Va.Si.Li-Lab, eine zentrale
Vermittlungskomponente und eine Menge containerisierter Rekonstruktionsmodule.
Das Frontend erfasst Kameradaten, bündelt sie zu Fragmenten und visualisiert
die vom Backend gelieferten Rekonstruktionsergebnisse. Die Vermittlungsschicht
koppelt diese Fragmente über einheitliche Nachrichtenstrukturen an die
Rekonstruktionsmodule und verteilt deren Ergebnisse szenenbasiert an die
Clients. Auf dieser modularen Streaming-Architektur bauen die in Kapitel~5
beschriebene Implementierung und die in Kapitel~6 analysierte Leistungsfähigkeit
des Systems auf.
