...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{lcclX}
        ...
        V1 -- Primitive & $5\times 5\times 5$ & 24 & 216 & Geometrische Grundformen, einfarbige Oberflächen \\
...
Die durchschnittliche Frame Rate des Unity-Clients wurde sowohl im Baseline-Betrieb ohne aktive Rekonstruktion als auch während der Rekonstruktions- und Visualisierungsphase gemessen (Abbildung~\ref{frame_rate}). Da sich die GPU-/CPU-Auslastung auf der Meta Quest 3 im laufenden Betrieb nur eingeschränkt messen ließ, wird die Performance hier ausschließlich über die FPS ausgewertet. Im Baseline-Betrieb erreicht der Client eine durchschnittliche Bildrate von 72\,fps.  
...
Es bestehen primät drei Limitationen. Erstens erreichen SLAM-Verfahren auf der verwendeten NVIDIA GTX 1070 Ti eine GPU-Auslastung von 98--100\% bei vollständiger VRAM-Auslastung (8~GB), was den parallelen Betrieb mehrerer Modelle einschränkt (vgl.~\ref{tab:gpu_resources}). Dieser Engpass ist hardwarespezifisch und würde sich mit moderneren GPUs deutlich reduzieren. Er unterstreicht jedoch die Relevanz der Architektur. Diese ermöglicht die direkte Skalierung auf leistungsfähigere Hardware ohne Code-Anpassungen. Zweitens fehlt eine persistente Zustandsverwaltung, sodass Container-Neustarts zum Datenverlust führen. Drittens erfordert die Integration von SLAM-Rekonstruktionen eine manuelle Skalierung durch die Nutzenden, da diese in modellinternen Koordinatensystemen vorliegen. Diese Einschränkungen sind nicht durch die Architektur bedingt, sondern spiegeln die Hardware-Beschränkungen der Evaluationsumgebung, Implementierungsentscheidungen und die fehlende Standardisierung zwischen Rekonstruktionsverfahren und VR-Tracking-Systemen wider.
