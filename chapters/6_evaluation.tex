...
Die Evaluation verfolgt drei zentrale Ziele, die direkt aus der in Kapitel~\ref{ch:introduction} formulierten Forschungsfrage und den in Kapitel~\ref{sec:andorderungen} definierten Anforderungen abgeleitet sind.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l X}
        ...
        Netzwerk & WiFi 5 Heimnetzwerk, 60~Mbit Internet-Verbindung \\
        \midrule
...
Praxistauglichkeit. Alle Modelle arbeiten während der Evaluation dabei mit einer Fenstergröße von \textit{9}. Eine kurze Übersicht über alle Testszenen findet sich in Tabelle~\ref{tab:test_scenes_overview}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{lcclX}
        ...
        V1 -- Primitive & $5\times 5\times 5$ & 24 & 216 & Grundformen, einfarbige Oberflächen \\
...
Das Schlafzimmer ist ein schmaler Raum mit Dachschräge und Arbeits- und Ruhebereich. Am einen Ende befindet sich ein Schreibtisch mit Bürostuhl und Monitor, am anderen Ende ein graues Sofa und ein TV-Board. Ein großes Dachflächenfenster sowie ein seitliches Fenster sorgen für viel Tageslicht. Der dunklere Bodenbelag und die hellen Wände erzeugen deutliche Kontraste, die für die Rekonstruktion von Kanten und Strukturen vorteilhaft sind.
...
(GPU-Verarbeitung) und $L_{\text{render}}$ die Zeit für das Rendering im Unity-Client (GLB-Import und Mesh-Visualisierung) bezeichnet.
...
Die Rekonstruktionen wurden anhand folgender Kriterien bewertet:
...
visualisiert. Das in Kapitel 5 beschriebene Visualisierungskonzept ermöglichte eine performante Darstellung auch bei größeren Meshes und Punktwolken mit bis zu 100\,000 Punkten.
...
Die Boxplots zeigen für alle Modelle und Szenen geringe Interquartilbereiche. Dies weist auf eine stabile und reproduzierbare Latenzcharakteristik des Systems hin. Aufgrund der geringen Varianz zwischen den drei Testläufen werden in den folgenden Analysen zur Zusammensetzung der Gesamtlatenz sowie zu den Datenvolumina die Messwerte der drei Testläufe aggregiert dargestellt. Dies ermöglicht eine kompaktere Präsentation ohne relevanten Informationsverlust.

Die Ergebnisse zeigen einen klaren Trend: Je komplexer die virtuele Szene wird (V1 \(\longrightarrow\) V3), desto höher fällt die Gesamtlatenz aus. Gleichzeitig fällt auf, dass SLAM-Verfahren in den realen Szenen (R1/R2) teils überraschend besser bzw. robuster abschneiden als in den virtuellen Szenen. Um die Gründe für diese Unterschiede besser einzugrenzen, wird die Gesamtlatenz im nächsten Schritt in ihre einzelnen Komponenten zerlegt.


\paragraph{Zusammensetzung der Gesamtlatenz}

Die beobachteten Latenzunterschiede zwischen den Testszenen lassen sich durch die Zerlegung der Gesamtlatenz in ihre konstituierenden Komponenten \(L_{\text{network}}\), \(L_{\text{inference}}\) und \(L_{\text{render}}\) analysieren. Mithilfe dieser Unterteilung ist es möglich, szenenabhängige Effekte von den fixen Overhead-Kosten der Netzwerkkommunikation und der Rendering-Pipeline zu trennen. Abbildung~\ref{fig:latency_stacked_bar} visualisiert die resultierende Zusammensetzung für alle Modelle und Testszenen.
...
Die Tabelle zeigt, dass die durchschnittlichen Fragmentgrößen für alle Szenen nahezu identisch sind (1{,}56--1{,}84~MB), mit Ausnahme von Szene \textit{V1} (0{,}82~MB). Diese Unterschiede sind ein Resultat der JPEG-Komprimierung, die je nach Bildinhalt unterschiedlich effektiv ist.

Die durchschnittlichen Ergebnisgrößen pro Fragment unterscheiden sich hingegen deutlicher zwischen den Modellen: VisFusion erzeugt mit $2{,}58$--$4{,}24~\text{MB}$ die größten Meshes im GLB-Format, während NeuralRecon mit $1{,}83$--$3{,}60~\text{MB}$ ähnlich große Meshes produziert. Die punktbasierten Modelle (MASt3R-SLAM, SLAM3R) liefern mit ihren 100{,}000 Punkten konstant ca. $1{,}60~\text{MB}$ zurück. Die Summe der genutzten Bandbreite über alle Fragmente variiert entsprechend der unterschiedlichen Fragmentanzahl pro Szene, wobei beispielsweise in Szene V3 insgesamt 88{,}70~MB für den Upload und zwischen 90{,}69~MB (MASt3R-SLAM) und 241{,}51~MB (VisFusion) für den Download gemessen wurden.
...
In den realen Szenen R1 und R2 liegen die Durchsätze bei allen Modellen vergleichsweise höher als in den virtuellen Szenen. So erreicht \textit{NeuralRecon} 0{,}48-0{,}45 fps, \textit{VisFusion} 0{,}38-0{,}31 fps, \textit{SLAM3R} 0{,}12 fps in beiden Szenen und \textit{MASt3R-SLAM} 0{,}10 bzw. 0{,}09 fps.
...
Da sich die GPU-/CPU-Auslastung auf der Meta Quest 3 im laufenden Betrieb nur eingeschränkt messen ließ, wird die Performance hier ausschließlich über die FPS ausgewertet. Die durchschnittliche Frame Rate des Unity-Clients wurde sowohl im Baseline-Betrieb ohne aktive Rekonstruktion als auch während der Rekonstruktions- und Visualisierungsphase gemessen (Abbildung~\ref{frame_rate}).  Im Baseline-Betrieb erreicht der Client eine durchschnittliche Bildrate von 72\,fps.  
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/frontend_fps.png}
...
Die Rekonstruktionen von NeuralRecon zeigten deutliche Lücken über alle Testszenarien hinweg. Besonders in Szene V1 wurden schräge Flächen vollständig ausgelassen, während horizontale und vertikale Strukturen zumindest teilweise erfasst wurden. In den komplexeren Szenen V2 und V3 setzte sich diese Lückenhaftigkeit fort. Neben den fehlenden Bereichen fielen auch vereinfachte Geometrien auf, wobei feine Details verloren gingen, während gröbere Strukturen noch erkennbar blieben. Positiv hervorzuheben ist hingegen die Qualität der tatsächlich erfassten Bereiche: Die Meshes wiesen glatte, geschlossene Oberflächen auf und waren frei von Fehlgeometrien oder Flimmerartefakten. Die globale Koherenz blieb durchgängig erhalten.

\begin{figure}[H]
    \centering
    \begin{subfigure}{0.49\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room00/gt00.png}
        \caption{Ground Truth}
    \end{subfigure}
    \begin{subfigure}{0.49\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room00/neucon00.png}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.49\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room01/gt00.png}
        \caption{Ground Truth Detail}
    \end{subfigure}
    \begin{subfigure}{0.49\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room01/neucon00.png}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \begin{subfigure}{0.49\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room00/visfusion00.png}
        \caption{VisFusion}
    \end{subfigure}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.49\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room01/visfusion00.png}
        \caption{Szene V2}
    \end{subfigure}
    \begin{subfigure}{0.49\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room02/visfusion00.png}
        \caption{Szene V3}
    \end{subfigure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \begin{subfigure}{0.49\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room00/slam3r00.png}
        \caption{SLAM3R Rekonstruktion}
    \end{subfigure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \begin{subfigure}{0.32\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room01/slam3r00.png}
        \caption{Szene V2}
    \end{subfigure}
    \begin{subfigure}{0.32\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room02/slam3r00.png}
        \caption{Szene V3}
    \end{subfigure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \begin{subfigure}{0.49\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room00/mast3r0.png}
        \caption{MASt3R-SLAM}
    \end{subfigure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \begin{subfigure}{0.32\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room01/mast3r00.png}
        \caption{Szene V2}
    \end{subfigure}
    \begin{subfigure}{0.32\linewidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room02/mast3r00.png}
        \caption{Szene V3}
    \end{subfigure}
    \caption{MASt3R-SLAM Performance-Vergleich über Szenen: V1, V2 und V3. Im Gegensatz zu SLAM3R bleibt die Konsistenz über die Szenen hinweg relativ stabil.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room03/neucon00.png}
        \caption{Neural Recon}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room03/visfusion00.png}
        \caption{MASt3R-SLAM}
    \end{subfigure}
    \caption{Rekonstruktionen der volumetrischen Verfahren in Szene R1. NeuralRecon 
    mit erheblichen Diskontinuitäten. VisFusion mit marginal verbesserter Vollständigkeit.}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room04/slam3r00.png}
        \caption{SLAM3R}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\linewidth]{images/ReconsTransparent/room04/mast3r00.png}
        \caption{MASt3R-SLAM}
    \end{subfigure}
    \caption{Rekonstruktionen der SLAM-basierten Verfahren in Szene R2. Obwohl beide Modelle einen hohen Detailgrad haben, liegt MASt3R-SLAM klar vorne}
    \label{fig:real_slam}
\end{figure}
...
Während sie in den synthetischen Szenen teilweise mit Artefaktbildung und verlängerter Inferrenz konfrontiert waren, erzielten sie in den realen Szenen hervorragende Rekonstruktionsergebnisse mit erhöhtem Detailgrad und geringen Diskontinuitäten. 
...
Der gemessene Integrationsaufwand von durchschnittlich 47--66 Zeilen Dockerfile und 205--401 Zeilen Worker-Code pro Modell (vgl.\ref{tab:integration_effort}) demonstriert die Effizienz der entwickelten Abstraktionsschicht. Diese Werte sind insofern aussagekräftig, als das sie ausschließlich den integrationsspezifischen Code umfassen, wobei die eigentlichen Rekonstruktionsmodelle unverändert bleiben und lediglich durch eine einheitliche Kommunikationsschicht gekapselt werden. Die geringe Varianz zwischen den Modellen (Dockerfile: $\pm 19$ LoC, Worker: $\pm 196$ LoC) deutet darauf hin, dass die Abstraktion stabil über unterschiedliche Modellarchitekturen hinweg funktioniert. Ein wesentlicher Teil des Worker-Codes entfällt dabei noch auf modellspezifische Datenvorverarbeitung, wie etwa die Transformation von Unity-Koordinaten\-systemen in die jeweiligen Modell-Konventionen, oder Inferrenz-Prozesse was als unvermeidlicher Aufwand einzustufen ist der in jeder Integrationsarchitektur anfallen würde.

Die Effizienz der Kommunikationsschicht zeigt sich in den kompakten Datenvolumina und der geringen Backend-Ressourcenauslastung. Das binäre Protokoll vermeidet den Overhead von ca. 33\%, der durch die Base64-Kodierung der Kamerabilder entstehen würde und erreicht durch JPEG-Komprimierung und GLB-Serialisierung Uploadgrößen von 0{,}82--1{,}84~MB und Downloadgrößen von 1{,}60--4{,}24~MB pro Fragment (vgl.~\ref{tab:data_volumes}). Die Unterschiede zwischen Modellen reflektieren deren Repräsentationsformen: Volumetrische Verfahren generieren dichte Meshes (2{,}19--4{,}24~MB), während SLAM-basierte Ansätze konstant kompaktere Punktwolken (1{,}60~MB) erzeugen. Der zentrale Router benötigt während der Ausführung lediglich 20\% CPU und 100~MB RAM (vgl.~\ref{tab:container_resources}), was die Effizienz der Architektur unterstreicht. Die Worker-Container weisen moderate CPU-Auslastungen von 100--250\% und einen RAM-Verbrauch von 1{,}8--2{,}8~GB auf, der primär durch die Modellgewichte und deren Aktivierungen während der Inferenz bestimmt wird.
...
Die modulare Architektur erfüllt ihre Kernziele auf technischer Ebene umfassend. Die erfolgreiche Integration von vier methodisch unterschiedlichen Rekonstruktionsverfahren mit verschiedenen Framework-Abhängigkeiten und Repräsentationsformen validiert das Containerisierungskonzept als praktikabel (vgl.~\ref{tab:integration_effort}). Die asynchrone WebSocket-Architektur gewährleistet eine robuste, latenzarme Datenstromverarbeitung ohne Verbindungsabbrüche in sechs Stunden Testbetrieb (vgl. ~\ref{par:e2e_functional}). Die typischerweise niedrige Basis-Latenz von unter 1000 ms für Netzwerk- und Rendering-Komponenten zeigt, dass die Gesamtleistung nahezu linear mit der Modell-Inferenzgeschwindigkeit skaliert. Somit wird ein zentrales Designziel modularer Architekturen erfüllt (vgl.~\ref{fig:latency_stacked_bar}). Die Integration in das Va.Si.Li-Lab gelang additiv ohne disruptive Eingriffe, was die Wiederverwendbarkeit für andere VR-Plattformen nahelegt.

Aus wissenschaftlicher Perspektive ermöglichte die Architektur systematische Erkenntnisse, die über isolierte Modell-Benchmarks hinausgehen. Die identifizierte Leistungsumkehr zwischen synthetischen und realen VR-Umgebungen (vgl. Tabelle~\ref{tab:fscore_all}, Abbildung~\ref{fig:real_volumetric} und Abbildung \ref{fig:real_slam}) wäre ohne eine vergleichende Evaluation unter identischen Bedingungen kaum systematisch erfassbar gewesen. Die Feststellung, dass die Ursache in SDK-Limitationen der Meta Quest 3 (fehlende Frame-Timestamps) und nicht in inhärenten Algorithmen-Schwächen liegt, zeigt den Mehrwert kontrollierter Vergleiche. Ebenso offenbarte die Evaluation spezifische Trade-offs: Volumetrische Verfahren sind mit 0{,}30 bis 0{,}62 Fragmenten pro Sekunde schneller, benötigen aber präzise Extrinsiken. SLAM-Verfahren sind mit 0{,}03 bis 0{,}13 Fragmenten pro Sekunde langsamer, aber robuster gegenüber Sensor-Artefakten (vgl. Tabelle~\ref{tab:throughput_results}). Die Architektur erlaubt es, solche Entscheidungen kontextspezifisch zu treffen, ohne dass Code-Anpassungen erforderlich sind.
\clearpage

Es bestehen primät drei Limitationen. Erstens erreichen SLAM-Verfahren auf der verwendeten NVIDIA GTX 1070 Ti eine GPU-Auslastung von 98--100\% bei vollständiger VRAM-Auslastung (8~GB), was den parallelen Betrieb mehrerer Modelle einschränkt (vgl.~\ref{tab:gpu_resources}). Dieser Engpass ist hardwarespezifisch und würde sich mit moderneren GPUs deutlich reduzieren. Er unterstreicht jedoch erneut die Relevanz der Architektur. Diese ermöglicht die direkte Skalierung auf leistungsfähigere Hardware ohne Code-Anpassungen. Zweitens fehlt eine persistente Zustandsverwaltung, sodass Container-Neustarts zum Datenverlust führen. Drittens erfordert die Integration von SLAM-Rekonstruktionen eine manuelle Skalierung durch die Nutzenden, da diese in modellinternen Koordinatensystemen vorliegen. Kapitel~\ref{sec:future_works} diskutiert mögliche Lösungsansätze für diese Limitationen.

Zusammenfassend zeigt sich, dass sich die entwickelte Architektur sehr gut als flexible Integrationsplattform für Forschungs- und Evaluationszwecke eignet. Sie ermöglicht den transparenten Austausch von Rekonstruktionsverfahren, die Identifizierung der Eignung für bestimmte Deployments und die Ableitung vergleichender Erkenntnisse unter kontrollierten Bedingungen. Für produktive VR-Anwendungen mit Echtzeitanforderungen empfiehlt sich eine kontextabhängige Verfahrenswahl: volumetrische Ansätze für synthetische Umgebungen oder präzise Kamera-Hardware bzw. SLAM-basierte Ansätze für Consumer-VR mit SDK-Limitationen. Die Modularität des Systems gewährleistet, dass solche Entscheidungen gezielt für das jeweilige Einsatzszenario getroffen werden können, ohne dass eine grundlegende Anpassung der Frontend- oder Backend-Implementierung erforderlich ist. Dies ist ein zentrales Kriterium für langfristig wartbare VR-Forschungsinfrastrukturen.