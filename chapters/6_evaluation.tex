...
\begin{table}[h]
    \centering
    \caption{Durchschnittliche Frame Rate im VR-Frontend}
    \label{tab:frame_rate}
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Testlauf} & \textbf{V1 [fps]} & \textbf{V2 [fps]} & \textbf{V3 [fps]} & \textbf{R1 [fps]} & \textbf{R2 [fps]}\\
        \midrule
        Baseline (ohne Rekonstruktion) & \textit{[XX]} & \textit{[XX]} & \textit{[XX]} & \textit{[XX]} & \textit{[XX]} \\
        Mit Rekonstruktion             & \textit{[XX]} & \textit{[XX]} & \textit{[XX]} & \textit{[XX]} & \textit{[XX]} \\
        \midrule
        \textbf{Differenz}             & \textit{[±X]} & \textit{[±X]} & \textit{[±X]} & \textit{[±X]} & \textit{[±X]} \\
        \bottomrule
    \end{tabular}
\end{table}
